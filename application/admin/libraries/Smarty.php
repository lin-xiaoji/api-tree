...
class Smarty
{
    ...
    {
    	$path = trim(dirname(str_replace($this->template_dir,'',$filename)),'/');
    	if ($path)
    	{
        ...
        $name = $this->compile_dir . '/' . $path . basename($filename) . '.php';
        $dir = dirname($name);
        ...
        foreach ($arr AS $val)
        {
            if (in_array($val, $scripts) == false)
            {
                ...
                else
                {
                    ...
                    {
                        $src = $this->js_path . $val .'?'.$this->js_ver;
                    }
    ...
    function smarty_insert_styles($args){
        ...
        foreach ($arr AS $val)
        {
            if (in_array($val, $csss) == false)
            {
                ...
                {
                	$val = $this->css_path . $val.'?'.$this->css_ver;
                }
